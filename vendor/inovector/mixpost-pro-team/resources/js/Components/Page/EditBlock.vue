...
<template>
    ...
    <DialogModal :show="modal"
                 ...
                 @close="attemptClose">
        <template #header>
            <span>{{ $t('page.edit_block') }}[<span class="font-semibold capitalize">{{ form.module }}</span>]</span>
        </template>
        ...
        <template #footer>
            <SecondaryButton @click="attemptClose" class="mr-xs rtl:mr-0 rtl:ml-xs">{{ $t('general.cancel') }}</SecondaryButton>
            <DangerButton @click="deleteBlock" class="mr-xs rtl:mr-0 rtl:ml-xs">{{ $t('general.delete') }}</DangerButton>
