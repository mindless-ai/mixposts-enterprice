...
const getDayTimes = (day) => {
    ...
    return sorted.map((item) => {
        return {
            ...
            formatValue: getTimeFormat(item.value)
        };
    });
}
...
const remove = (dayIndex, timeValue) => {
    const getTimeIndex = props.times[dayIndex].times.findIndex(item => item.value === timeValue);

    props.times[dayIndex].times.splice(getTimeIndex, 1);
}
</script>
<template>
