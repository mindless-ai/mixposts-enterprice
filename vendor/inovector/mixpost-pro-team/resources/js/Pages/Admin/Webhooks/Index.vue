...
const deleteWebhooksAfterConfirmed = (dialog) => {
    ...
    router.delete(route(`${routePrefix}.system.webhooks.deleteMultiple`), {
        data: {
...
watch(() => cloneDeep(filter.value), throttle(() => {
    router.get(route('mixpost.system.webhooks.index'), pickBy(filter.value), {
        preserveState: true,
        only: ['records', 'filter']
    });
}, 300))
...
<template>
    ...
    <div class="w-full mx-auto row-py">
        ...
        <div class="w-full row-px mt-lg">
            <Link :href="route(`${routePrefix}.system.webhooks.create`)">
