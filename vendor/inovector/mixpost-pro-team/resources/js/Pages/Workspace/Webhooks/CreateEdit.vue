...
const store = (login = false) => {
    form.transform((data) => ({
        ...data,
        login
    })).post(route(`${routePrefix}.webhooks.store`, {'workspace': workspaceCtx.id}), {
        onError: (errors) => {
...
const update = () => {
    form.put(route(`${routePrefix}.webhooks.update`, {'workspace': workspaceCtx.id, webhook: props.record.id}), {
        preserveScroll: true,
        onError: (errors) => {
